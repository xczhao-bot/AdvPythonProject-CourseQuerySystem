...
class MainWin(tk.Tk):
    ...
    def __init__(self):
        ...
        self.title("Udemy")
        # top label
        self.topLabel = tk.Label(self, text="Udemy IT Courses Query System", font=("Helvetica", 18, "bold"), fg=UdemyColorDict["red"])
        self.topLabel.grid(row=0, columnspan=3)
        
        # 3 buttons for overview options
        self.b1 = tk.Button(self, text="Course\nSummary", height=4, width=12, fg=UdemyColorDict["mombasa"],
                            command=lambda: self.plot(1))
        self.b1.grid(row=1, column=0)
        self.b2 = tk.Button(self, text="Categories", height=4, width=12, fg=UdemyColorDict["mombasa"],
                            command=lambda: self.plot(2))
        self.b2.grid(row=1, column=1)
        self.b3 = tk.Button(self, text="Language\n&\nLevels", height=4, width=12, fg=UdemyColorDict["mombasa"],
    ...
    def plot(self, option):
        optionD = {1: backend_yueqi.plotSummaryChart,
                   2: backend_yueqi.plotCategoryChart,
                   3: backend_yueqi.plotLevelChart}
    ...
    def _driver(self):
        ...
        # get data from database (list of list)
        # use TreeView to present
        ...
        search = f'%{self.E.get()}%'
        print(level, subcategory, search)
        
        self.cur.execute(f'''SELECT {self.TB}.id, {self.TB}.title, {self.TB_LEVEL}.level, {self.TB}.price, 
                         {self.TB}.rating, {self.TB}.num_subscribers, {self.TB}.num_reviews, {self.TB}.last_update_date, {self.TB}.contents  
...
class Treeview(tk.Toplevel):
    ...
    def __init__(self, master, lst):
        ...
        # define columns
        self.tree["columns"] = ("id", "title", "level", "price", "rating", "num_subscribers", "num_reviews", "last_update_date", "contents")
        ...
        self.button = tk.Button(self, text="SAVE", command=self.selection, fg=UdemyColorDict["mombasa"])
        self.button.grid()
        # add style
        style = ttk.Style()
    ...
    def selection(self) :
        print(self.tree.selection())  # return a tuple of the row number
    ...
    def treeview_sort_column(self, tree, col, reverse):
        ...
        self.tree.heading(col, text=col, command=lambda _col=col: self.treeview_sort_column(self.tree, _col, not reverse))
